...
public:
    ...
    static constexpr inline expression_kind tag = expression_kind::where;

    /**
     * @brief creates a new object.
     * @param condition the condition expression
     */
    explicit where(util::unique_object_ptr<scalar::expression> condition) noexcept;
     ...
     */
    explicit where(scalar::expression&& condition) noexcept;
     ...
     */
    explicit where(where const& other, util::object_creator creator);
     ...
     * @param creator the object creator
     */
    explicit where(where&& other, util::object_creator creator);
    ...
    where* clone(util::object_creator creator) const& override;
    where* clone(util::object_creator creator) && override;
     ...
     * @return this
     */
    where& condition(util::unique_object_ptr<scalar::expression> condition) noexcept;
     ...
     */
    friend bool operator==(where const& a, where const& b) noexcept;
     ...
     * @return the output
     */
    friend std::ostream& operator<<(std::ostream& out, where const& value);
 ...
 */
template<> struct type_of<where::tag> : util::meta_type<where> {};
