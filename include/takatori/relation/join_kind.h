...
enum class join_kind {
    ...
    full_outer,
    /// @brief left outer join, at most one row per left row.
    left_outer_at_most_one,
 ...
 */
constexpr std::string_view to_string_view(join_kind value) noexcept {
    ...
    switch (value) {
        ...
        case kind::left_outer_at_most_one: return "left_outer_at_most_one"sv;
...
auto dispatch(Callback&& callback, join_kind tag_value, Args&&... args) {
    using kind = join_kind;
    switch (tag_value) {
        ...
        case kind::left_outer_at_most_one: return util::enum_tag_callback<kind::left_outer_at_most_one>(std::forward<Callback>(callback), std::forward<Args>(args)...);
