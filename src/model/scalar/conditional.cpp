...
conditional::conditional(
    ...
    , default_expression_(bless_element(*this, std::move(default_expression)))
{}
...
conditional::conditional(conditional const& other, util::object_creator creator)
    : conditional(
            forward(creator, other.alternatives_),
            forward(creator, other.default_expression_))
{}

conditional::conditional(conditional&& other, util::object_creator creator)
    : conditional(
            forward(creator, std::move(other.alternatives_)),
            forward(creator, std::move(other.default_expression_)))
{}
...
tree_fragment_vector<conditional::alternative>& conditional::alternatives() noexcept {
    return alternatives_;
}

tree_fragment_vector<conditional::alternative> const& conditional::alternatives() const noexcept {
    return alternatives_;
}
...
conditional& conditional::default_expression(util::unique_object_ptr<expression> default_expression) noexcept {
    return assign_element(*this, default_expression_, std::move(default_expression));
}

util::unique_object_ptr<expression> conditional::release_default_expression() noexcept {
    return release_element(std::move(default_expression_));
}
