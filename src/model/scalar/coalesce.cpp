...
#include "model/tree_element_vector_forward.h"
...
coalesce::coalesce(coalesce const& other, util::object_creator creator)
    : coalesce(forward(creator, other.alternatives_))
{}

coalesce::coalesce(coalesce&& other, util::object_creator creator)
    : coalesce(forward(creator, std::move(other.alternatives_)))
{}
...
tree_element_vector<expression>& coalesce::alternatives() noexcept {
    return alternatives_;
}

tree_element_vector<expression> const& coalesce::alternatives() const noexcept {
    return alternatives_;
}
