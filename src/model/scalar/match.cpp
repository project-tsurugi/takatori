...
match::match(
    ...
    , input_(bless_element(*this, std::move(input)))
    , pattern_(bless_element(*this, std::move(pattern)))
    , escape_(bless_element(*this, std::move(escape)))
{}
...
match::match(match const& other, util::object_creator creator) noexcept
    : match(
            ...
            forward(creator, other.input_),
            forward(creator, other.pattern_),
            forward(creator, other.escape_))
{}

match::match(match&& other, util::object_creator creator) noexcept
    : match(
            other.operator_kind_,
            forward(creator, std::move(other.input_)),
            forward(creator, std::move(other.pattern_)),
            forward(creator, std::move(other.escape_)))
{}
...
util::unique_object_ptr<expression> match::release_input() noexcept {
    return release_element(std::move(input_));
}

match& match::input(util::unique_object_ptr<expression> input) noexcept {
    return assign_element(*this, input_, std::move(input));
}
...
util::unique_object_ptr<expression> match::release_pattern() noexcept {
    return release_element(std::move(pattern_));
}

match& match::pattern(util::unique_object_ptr<expression> pattern) noexcept {
    return assign_element(*this, pattern_, std::move(pattern));
}
...
util::unique_object_ptr<expression> match::release_escape() noexcept {
    return release_element(std::move(escape_));
}

match& match::escape(util::unique_object_ptr<expression> escape) noexcept {
    return assign_element(*this, escape_, std::move(escape));
}
