...
class time_point_datetime_test : public ::testing::Test {};

using unit = time_point::time_unit;

TEST_F(time_point_datetime_test, epoch) {
    time_point tp;
    EXPECT_EQ(tp.elapsed(), unit(0));
    EXPECT_EQ(tp.date(), date());
    EXPECT_EQ(tp.time(), time_of_day());
}

TEST_F(time_point_datetime_test, unix_time) {
...
TEST_F(time_point_datetime_test, date_time) {
    time_point tp {
        date(1901, 2, 3),
...
TEST_F(time_point_datetime_test, output) {
