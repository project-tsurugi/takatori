...
namespace takatori::serializer::base128v {
...
public:
    ...
    std::string dump_unsigned(std::uint64_t value) {
        std::string result {};
        result.resize(9);
        util::buffer_view view { result.data(), result.size() };
    ...
    std::optional<std::uint64_t> restore_unsigned(std::string sequence) {
        util::buffer_view view { sequence.data(), sequence.size() };
        auto iter = view.begin();
...
TEST_F(base128v_test, write_overflow) {
    ...
    {
        buf.resize(0);
        util::buffer_view view { buf.data(), buf.size() };
        auto iter = view.begin();
...
TEST_F(base128v_test, read_undeflow) {
    {
        ...
        util::buffer_view view { buf.data(), buf.size() - 1};
        auto iter = view.begin();
