...
`store`
~ 入力リレーションに含まれる唯一の行の値を、指定された変数の列に代入する。
  ...
  * プロパティ
    * `columns+` - 出力する列の一覧
      ...
      * `variable` - 代入先の変数
  * その他の特性
    * 入力リレーションに含まれる行が1行でない場合、実行時エラーとなる
    * 代入先の変数は、ホスト変数でなければならない
    * 代入先のホスト変数を同一の実行計画内から参照することはできない (未定義動作)
...
----
* note:
  * ホスト変数周辺は https://www.postgresql.jp/document/9.6/html/ecpg-variables.html と矛盾がないように
