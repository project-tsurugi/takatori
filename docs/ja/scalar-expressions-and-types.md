...
~ 固定小数点数
  ...
  * パラメーター
    ...
    * `scale` - 小数部の桁数 (10進数)
  * 特性
    * `precision` が未指定の場合、桁数に上限がない
    * `0 <= scale <= precision` でなければならない
...
~ 固定長文字列
  ...
  * パラメーター
    * `length` - 文字列の長さ
      * 長さの計算方法は `national` パラメータによって変化する
        * `national=false` - システムエンコーディングにおける文字列のオクテット数を長さとする
        * `national=true` - 文字数を長さとする
    * `national` - UTF-8を利用するかどうか
  * 特性
    * 既定の文字数に満たない文字列は何らかの文字でパディングされる
    * `national` の指定によって演算子の多重定義の解決方法が異なる
...
~ 行ID
  ...
  * パラメータ
    * `defining` - 行IDを定義した要素の識別子
  * 特性
    * コンピューター上の表現は実行エンジンが決める
    * `defining` が異なる2つの行IDは比較不可能

`placeholder`
~ 未解決のプレースホルダーであることを表す
...
* `z`, `z'` のいずれもタイムゾーンの設定がない - `z''` は未指定
* `z`, `z'` のいずれか一方のみにタイムゾーンの設定がある - `z''` はそのタイムゾーン
* `z`, `z'` の両方に同一のタイムゾーンの設定がある - `z''` はそのタイムゾーン
* `z`, `z'` の両方に異なるタイムゾーンの設定がある - `z''` は `UTC` (TBD)
...
| `character varying` | D | D | D | D | D | D | D | D | D | v | D | D | D | D | D | D | D | D |
|               `bit` | - | - | - | - | - | - | - | - | v | v | v | - | - | - | - | - | - | - |
|       `bit varying` | - | - | - | - | - | - | - | - | v | v | - | v | - | - | - | - | - | - |
|              `date` | - | - | - | - | - | - | - | - | v | v | - | - | v | - | v | - | - | - |
|       `time_of_day` | - | - | - | - | - | - | - | - | v | v | - | - | - | v | v | - | - | - |
...
なお、ここまでに紹介したすべての型変換は、実行時には明示的な `cast` 式として出現する。
...
1. システム変数またはセッション変数のうち、実行前に参照可能であり、かつ常に同じ値を返す
...
~ 型変換
  ...
  * 特性
  ...
  `character` ~ `character`
  ~ 文字列を表す値 `v` を `T` の `national` に定められた方法で再度エンコードした値を `v'` とおく。

    `v'` の長さが `T` の `length` と等しい場合、 `v'` が式の評価結果となる。
  ...
  ~ 文字列を表す値 `v` を `T` の `national` に定められた方法で再度エンコードした値を `v'` とおく。
    ...
    `v'` の長さが `T` の `length` にと等しいか小さい場合、 `v'` が式の評価結果となる。
  ...
  ~ 文字列を表す値 `v` を `T` の `national` に定められた方法で再度エンコードした値を `v'` とおく。
    ...
    `v'` の長さが `T` の `length` にと等しいか小さい場合、 `v'` が式の評価結果となる。
    そうでない場合、 `v'` から `T` の `length` まで末尾から文字を取り除いたものが式の評価結果となる。
    文字を取り除く場合、 **精度が失われる**。
  ...
  ~ 時刻を表す値 `v` に対し、 `T` の時計における `v` と同じ時刻の `1900-01-01` の日時を式の評価結果とする。
  ...
  ~ 日時を表す値 `v` に対し、 `T` の時計における `v` の時刻を式の評価結果とする（`T` のタイムゾーンも引き継ぐ）。
    `T` の時計における `v` の日付が `1900-01-01` 以外の場合、 **精度が失われる**。
